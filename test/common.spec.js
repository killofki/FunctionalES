...
describe( '[ curry2 ]', q => { 
	...
	it( 'curry2 를 통해 리턴된 함수는 인자가 2개 미만이면 함수를 리턴하고, 2개 이상이면 해당 함수를 실행한다.', q => { 
		const adder = curry2( ( a, b ) => a + b ); 
...
describe( '[ pipe ]', [ self => { 
	...
	it( 'pipe 로 만든 함수를 실행하면, pipe 에 전달되었던 함수들을 순차적으로 실행하면서 값을 누적한다. 그러다 promise를 만나면, 모든 함수들의 누적 결과를 만들기로 약속된 promise를 리턴한다.', async q => { 
		self .timeout( 2000 ); 
		...
		expect( await p2 ) .to .eql( 6 ); 
		  ...
		  ( f, v ) => f( v )
		, F => function ( ... ar ) { F( this, ... ar ); } // minorThis 
		) 
	); 
